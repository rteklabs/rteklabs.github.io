...
async function boot(){const id=new URLSearchParams(location.search).get('id');if(!validId(id)){$('mapPlaceholder').textContent='Invalid property map link.';return;}try{const res=await fetch('maps/'+encodeURIComponent(id)+'.json',{cache:'no-store'});if(!res.ok)throw new Error(res.status===404?'Property map not found.':'Could not load property map.');data=await res.json();if(!data||data.id!==id||!data.home||!Array.isArray(data.pois))throw new Error('Property map data is invalid.');renderClient();loadGoogle();}catch(e){$('mapPlaceholder').textContent=e.message;}}
