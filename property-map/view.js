...
let data=null,map=null,infoWindow=null,lang='en',homeMarker=null,homeHaloMarker=null,homeGlowMarker=null,poiMarkers=[];
...
function clearMarkers(){if(homeMarker)homeMarker.setMap(null);if(homeHaloMarker)homeHaloMarker.setMap(null);if(homeGlowMarker)homeGlowMarker.setMap(null);poiMarkers.forEach(m=>m&&m.setMap(null));homeMarker=null;homeHaloMarker=null;homeGlowMarker=null;poiMarkers=[];}
function renderMap(fit){if(!map||!data)return;clearMarkers();const bounds=new google.maps.LatLngBounds();let count=0,home=homeData();
  if(home.lat!=null&&home.lng!=null){const pos={lat:home.lat,lng:home.lng};homeGlowMarker=new google.maps.Marker({map,position:pos,clickable:false,zIndex:997,icon:{path:google.maps.SymbolPath.CIRCLE,scale:28,fillColor:'#2563eb',fillOpacity:.08,strokeColor:'#60a5fa',strokeOpacity:.12,strokeWeight:2}});homeHaloMarker=new google.maps.Marker({map,position:pos,clickable:false,zIndex:998,icon:{path:google.maps.SymbolPath.CIRCLE,scale:24,fillColor:'#2563eb',fillOpacity:.13,strokeColor:'#ffffff',strokeOpacity:.48,strokeWeight:2}});homeMarker=new google.maps.Marker({map,position:pos,zIndex:999,title:(home.name||'Property')+' — Main property',icon:{path:google.maps.SymbolPath.CIRCLE,scale:19,fillColor:'#0f172a',fillOpacity:1,strokeColor:'#ffffff',strokeOpacity:1,strokeWeight:4},label:{text:'⌂',color:'#ffffff',fontSize:'18px',fontWeight:'900'}});homeMarker.addListener('click',()=>{infoWindow.setContent('<strong>'+esc(home.name||'Property')+'</strong><br><span style="font-size:11px;font-weight:700;color:#64748b">MAIN PROPERTY</span><br>'+esc(home.address||'')+'<br><a target="_blank" rel="noopener" href="'+esc(googleUrl(home))+'">Open in Google Maps ↗</a>');infoWindow.open(map,homeMarker);});bounds.extend(pos);count++;}
